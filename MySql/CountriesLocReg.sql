...
SELECT * FROM regions WHERE region_id NOT IN (SELECT DISTINCT REGION_ID FROM countries);

--display the countries there no states
SELECT * FROM countries WHERE COUNTRY_ID NOT IN (SELECT DISTINCT COUNTRY_ID FROM locations);

--display region name,countryname and state name
SELECT r.region_name, c.COUNTRY_NAME, l.STATE_PROVINCE 
